...
本节配套代码按检索入口拆分，不固定每种模式的代码数量：

- `retrieval_modes.py`：用规则模拟 query router，理解不同问题为什么走不同入口。
- `local_graphrag_retriever.py`：从 chunk 命中出发拼接实体和邻域事实。
- `neo4j_local_retriever.py`：给出真实 Neo4j Driver 查询版本，演示向量命中后如何扩展图邻域。
- `global_community_retriever.py`：用社区摘要构造全局问题上下文。
- `hybrid_graphrag_pipeline.py`：把向量、全文、图邻域和全局摘要候选合并成带来源证据。
- `graph_cypher_qa_demo.py` 与 `cypher_qa_chain_langchain.py`：对比白名单 Cypher 模板和 LangChain GraphCypherQAChain 的用法与风险。
...
`hybrid_graphrag_pipeline.py` 里把这个过程拆成候选证据、去重、排序、上下文拼装四步。真实系统可以把最后的上下文交给任意 Chat Model，但检索代码必须先保证来源、分数和证据类型可追踪。
...
GraphRAG 工程案例的重点不是“永远使用图”，而是根据问题选择检索模式。

```python
def route_query(question):
    q = question.lower()
    if any(word in question for word in ["整体", "主题", "总结", "全局"]):
        return "global_vector"
    if any(word in question for word in ["影响", "关联", "路径", "为什么"]):
        return "graph_vector_fulltext"
    if any(word in question for word in ["数量", "多少", "排名"]):
        return "graph"
    if any(word in q for word in ["id", "sku", "合同号"]):
        return "fulltext"
    return "vector"

def build_context(question, retrievers):
    mode = route_query(question)
    context = retrievers[mode].retrieve(question)
    return {"mode": mode, "context": context}
```

这个路由器很粗糙，但它体现了生产系统的判断：GraphRAG 是检索策略组合，不是单一按钮。

## 小结

GraphRAG 的价值不是“更酷”，而是把检索从相似文本提升到结构化上下文。它能让 LLM 回答更可追踪、更适合多跳推理，但也带来更高的数据建模和维护成本。

评估 GraphRAG 代码是否足够完整时，可以看它是否回答了这些问题：

- 向量、全文、图查询和社区摘要的候选如何合并？
- 每条证据是否带有来源、score、类型和可展示文本？
- 多跳扩展有没有限制 hop、limit 和关系类型，避免路径爆炸？
- Text-to-Cypher 是否限制只读查询，是否防止 LLM 生成写入或删除语句？
