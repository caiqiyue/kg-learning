...
ENTITY_DICTIONARY = {
    "知识图谱构建器": {"id": "knowledge-graph-platform", "type": "Project"},
...
def extract_relationships(text: str, entities: list[dict]) -> list[dict]:
    ...
    if "knowledge-graph-platform" in entity_ids and "LangChain" in entity_ids and "使用" in text:
        relationships.append({
            "source": "knowledge-graph-platform",
            "type": "USES",
            "target": "LangChain",
    ...
    if "knowledge-graph-platform" in entity_ids and "PDF" in entity_ids and "抽取" in text:
        relationships.append({
            "source": "knowledge-graph-platform",
            "type": "EXTRACTS_FROM",
            "target": "PDF",
    ...
    if "knowledge-graph-platform" in entity_ids and "Neo4j" in entity_ids and "写入" in text:
        relationships.append({
            "source": "knowledge-graph-platform",
            "type": "STORES_IN",
            "target": "Neo4j",
